...
import { BasketballStandingsTable } from "../components/BasketballStandingsTable";
import { Helmet } from "react-helmet";
import { useToast } from "@/context/ToastContext";

// Define the interface for Basketball League based on current understanding
interface BasketballLeagueDetail {
  _id?: string;
  league_id: number;
  country: string;
  name: string;
...
const BasketballLeaguePage = () => {
  ...
  // We prioritize the league resolved from the name param if it exists, otherwise fall back to ID logic.
  // But wait, name param is encoded name.

  const [league, setLeague] = useState<BasketballLeagueDetail | null>(null);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError(null);
      try {
        let resolvedLeague: BasketballLeagueDetail | null = null;

        // Strategy:
        // 1. If we have a 'name' param, use getBasketballLeagueByName.
        // 2. If valid response, that's our league.
        // 3. If no 'name' but 'leagueIdFromQuery', fetch by ID.

        if (name) {
          const decodedName = decodeURIComponent(name);
          const res = await getBasketballLeagueByName(decodedName);
          if (res?.success && res?.responseObject?.item) {
            const items = res.responseObject.item;
            // Pick best match or first
            resolvedLeague = Array.isArray(items) ? items[0] : items;
          }
        }

        if (!resolvedLeague && leagueIdFromQuery) {
          const res = await getBasketballLeagueById(leagueIdFromQuery);
          const items = res?.responseObject?.item;
          resolvedLeague = Array.isArray(items) ? items[0] : items;
        }
  ...
  const pageTitle = useMemo(
    () => `${leagueName} | League Profile | TikiAnaly`,
    [leagueName],
  );
  const pageDescription = useMemo(
    () => `Standings and league details for ${leagueName}.`,
    [leagueName],
  );

  return (
    <div className="dark:bg-[#0D1117] min-h-screen transition-colors duration-200">
      {isShareOpen ? (
        <div
          ...
          <div className="relative w-full max-w-lg rounded-2xl bg-white dark:bg-[#0D1117] border border-snow-200 dark:border-snow-100/10 shadow-2xl">
            <div className="flex items-start justify-between gap-4 px-5 pt-5">
              <div className="min-w-0">
                <p className="theme-text font-bold text-base">
                  Share this profile
                </p>
                <p className="text-neutral-m6 text-sm mt-1">
                  Copy the link below to share this page with friends or on
                  social media.
                </p>
              </div>
              <button
          ...
          <div className="w-full backdrop-blur-3xl h-full min-h-[220px] md:min-h-0 page-padding-x pb-4 md:pb-0 relative z-0">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 pb-2">
              <div className="flex items-center gap-4">
                {resolvedLeagueId ? (
                  <div className="bg-white p-4   rounded-2xl">
                    {/* Using Basketball logo placeholder/logic since GetLeagueLogo might be strictly football or generic */}
                    {/* Assuming GetLeagueLogo handles generic fallback */}
                    <div className="w-20 h-20 md:w-28 md:h-28 flex items-center justify-center bg-brand-primary/10 text-brand-primary rounded-xl text-3xl font-bold">
                      {leagueName.charAt(0)}
                    </div>
                  </div>
                ) : (
                ...
                <div className="min-w-0">
                  <p className="font-extrabold text-2xl md:text-3xl text-white whitespace-normal break-words">
                    {leagueName}
                  </p>
              ...
              <div className="text-white text-sm">
                ...
                {error ? (
                  <span className="text-ui-negative">{error}</span>
                ) : null}
              </div>
            </div>
          </div>
      ...
      <div className="page-padding-x">
        {!name && !leagueIdFromQuery && (
          <div className="my-4 block-style p-3 rounded theme-text text-center text-neutral-n4 dark:text-neutral-m6">
            Select a league from the sidebar to view details.
          </div>
        )}

        {activeTab === "standings" ? (
          <div className="my-8">
            <BasketballStandingsTable leagueId={resolvedLeagueId} />
          </div>
        ) : null}
      </div>
